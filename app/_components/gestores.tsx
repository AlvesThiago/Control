...
function GestoresPage() {
  ...
  const RegistroGestor = async (gestor: string, setorGestor: string, cracha: string) => {
    const response = await fetch("/api/gestores", {
      ...
      body: JSON.stringify({ gestor, setorGestor, cracha }),
    })

    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.error || "Falha ao registrar o gestor")
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await RegistroGestor(nome, setor, idCracha)
