...
function Atribuir() {
  ...
  const [users, setUsers] = useState(["", "", "", "", "", ""]) // Inicializando com seis campos vazios
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        ...
        const fetchedAssignments: Assignment[] = await Promise.all(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const atribuicao = async (nomes: string[], serialNumber: string) => {
    try {
      const notebookExistente = await db
        .select()
        .from(Notebooks)
        .where(eq(Notebooks.serialNumber, serialNumber))
        .execute()
      ...
      const [nomet1, nomet2, nomet3, nomet4, nomet5, nomet6] = nomes

      const atribuirResult = await db
        .insert(AtribuirNote)
        .values({
  ...
  const updateAssignment = async () => {
    ...
    try {
      ...
      const notebookExistente = await db
        .select()
        .from(Notebooks)
        .where(eq(Notebooks.serialNumber, editingAssignment.notebook))
        .execute()
      if (notebookExistente.length === 0) {
        setNotification({ message: "Notebook não encontrado!", type: "error" })
        return
      ...
      // Validate each user
      for (const nome of [nomet1, nomet2, nomet3, nomet4, nomet5, nomet6].filter(Boolean)) {
        const usuarioExistente = await db.select().from(Usuarios).where(eq(Usuarios.nome, nome)).execute()
        if (usuarioExistente.length === 0) {
          setNotification({ message: `Usuário ${nome} não encontrado!`, type: "error" })
          return
        ...
        setAssignments(
          assignments.map((assignment) =>
            assignment.id === editingAssignment.id
              ? { ...assignment, users: [nomet1, nomet2, nomet3, nomet4, nomet5, nomet6].filter(Boolean) }
              : assignment,
          ),
        )
  ...
  const deleteAssignment = async (id: number) => {
    try {
      ...
      const result = await db
        .delete(AtribuirNote) // Especificando explicitamente a tabela
